...
export class DevMenu {
  ...
  killAll() { const s = this.game.session; if (!s) return; for (const e of [...s.director.enemies]) { if (e.dead || e.type?.ally) continue; try { e.takeDamage(1e6, { zone: 'head', point: (e.hitCenter || e.position).clone(), normal: new THREE.Vector3(0, 1, 0), attackerId: 0, weaponId: 'dev', dir: new THREE.Vector3(0, 0, 1) }); } catch (err) { try { e.die?.({}); } catch { /* ignore */ } } } }
