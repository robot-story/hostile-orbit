...
export function buildSoldier(styleName = 'vanguard', opts = {}) {
  ...
  if (customKey && CUSTOM.body[customKey]) { try { applyCustomBody(model, CUSTOM.body[customKey], { neon: customKey === 'vanguard' ? style.neon : '#ff3b1f', tint: customKey === 'vanguard' ? null : '#5e6068', noLegs: customKey === 'vanguard', ballRadius: 0.5, torsoScale: 1.16 }); } catch (e) { console.warn('[glb] custom body failed', e); } }
  return model;
}
