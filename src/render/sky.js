...
export function createCelestials(variant = 'khepri') {
  ...
  colossus.position.set(760, 400, -1250); colossus.rotation.y = -0.6; colossus.scale.setScalar(0.75); colossus.userData.baseX = 760;
  g.add(colossus);
  g.userData.update = (t, dt) => {
    colossus.position.x = colossus.userData.baseX - (t * 0.6) % 900; colossus.position.y = 400 + Math.sin(t * 0.25) * 8; bodyC.scale.y = 0.7 + Math.sin(t * 0.5) * 0.03;
