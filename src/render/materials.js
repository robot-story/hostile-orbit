...
export const Mat = {
  ...
  alienMembrane: (color = COLORS.green) => cached('membrane' + color, () => useGenerated(new THREE.MeshStandardMaterial({ map: Tex.membrane(), color: color, emissive: color, emissiveIntensity: 0.9, transparent: true, opacity: 0.75, side: THREE.DoubleSide, roughness: 0.4, depthWrite: false }), 'tex_membrane', [1, 1])),
  blackGlass: () => cached('blackGlass', () => useGenerated(new THREE.MeshPhysicalMaterial({ color: '#05060a', roughness: 0.05, metalness: 0.4, clearcoat: 1, emissive: COLORS.violet, emissiveIntensity: 0.25 }), 'tex_black_glass', [1, 1], { color: '#5a5c66', roughness: 0.12 })),
